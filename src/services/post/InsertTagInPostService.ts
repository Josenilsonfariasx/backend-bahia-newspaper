...
class InsertTagInPostService {
  async execute({ post_id, tag_id}: PostRequest) {
    try { 
      ...
      if(tagAlreadyInserted) throw new Error('Tag already inserted in this post.')
      const postUpdated = await prismaClient.post.update({ where: { id: post_id }, include:{tags:true, categories:true}, data: { tags: { connect: { id: tag_id } } } })
      return postUpdated
    } catch (error) {
      throw new Error('Failed to insert tag in Post: '+error.message)
    }
    
  }
}
export { InsertTagInPostService };